...
import { Bookmark, CirclePlay, Dot, Film, Play, Star } from 'lucide-react';
...
const PopularWeek = () => {
        ...
        try {
            async function fetchPopular() {
                ...
                const [popular, movieGenres, tvGenres] = [
                    (await api.get('/trending/all/week'))?.data?.results?.sort((a, b) => b.popularity - a.popularity),
    ...
    return (
        <div>
            ...
            <div className='flex overflow-auto hide-scrollbar gap-16 mt-4'>
                {
                    data.length > 0 ? data.map((show, i) => <Link href={`/${show.media_type}/${show.id}`} className='shrink-0 flex items-center gap-4 ' key={i}>
                        <span className='text-5xl font-bold'>{i + 1}</span>
                        <Image src={`https://image.tmdb.org/t/p/w300${show.poster_path}`}
                        ...
                        <div className='text-sm space-y-1 '>
                            <span className='border-[1px] border-stone-600 rounded-lg px-3 py-1 mb-2 inline-block'>{show.original_language}</span>
                            <h2 className='font-semibold text-xl'>{show.title ? show.title : show.name}</h2>
                            ...
                            <div className='flex items-center text-sm gap-1'>
                                ...
                                <div>
                                    ...
                                    <span key={i} className='text-gray-400 capitalize'>{show.media_type}  </span>
                                </div>
                            </div>
                        </div>
                    </Link>)
            ...
            {
                popular?.media_type ? <div className=' md:h-96 overflow-hidden mt-16 rounded-2xl relative'>
                    <Link href={`/${popular.media_type}/${popular.id}`}>
                        <Image src={`https://image.tmdb.org/t/p/original${popular.backdrop_path}`}
                        ...
                        <div className=' z-[999] absolute top-2 md:top-10 md:px-20 md:mt-10 space-y-1'>
                            ...
                            <div className='flex items-center gap-1'>
                                <div className='flex items-center text-sm gap-1'>
                                    ...
                                    <div>
                                        <span className='font-semibold'>{parseFloat(popular.vote_average).toFixed(1)}  </span>
                                    </div>
                                </div>
                                <span>|</span>
                                <div className='flex text-stone-400'>
                                    {/* <Film className='' stroke='gray' /> */}
                                    {popular?.genres?.slice(0, 2)?.map((g, i, arr) => <span key={i} className='flex items-center text-nowrap flex-nowrap' >
                                        <span>{g.name}</span> <Dot className={`${i + 1 === arr.length ? 'hidden' : 'inline'} -mx-1`} />
                                    </span>)}
                                </div>
                    ...
                    <div className='h-80 md:h-96 overflow-hidden mt-16 rounded-2xl relative bg-stone-600 animate-pulse '>
                        ...
                        <div className=' z-[999] absolute top-10 md:px-20 mt-10 space-y-1'>
                            <h1 className='text-3xl font-bold bg-stone-600 W-32 h-8 rounded' > </h1>

                            <div className='bg-stone-600 w-40 rounded mt-3 h-4'>
