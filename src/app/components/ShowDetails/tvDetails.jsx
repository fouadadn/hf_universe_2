...
const TvDetails = ({ id, type, seasonNum }) => {
        ...
        try {
            ...
            async function fetchMovies() {
                const [show, casts, video, recommendationss, similar] = [
                    ...
                    (await api.get(`/tv/${id}/season/${seasonNum}/videos`)).data?.results,
    ...
    useEffect(() => {
        async function fetchSeasonData() {
            const season = (await api.get(`/tv/${id}/season/${seasonNum}`)).data
            ...
            <div className="-mt-32">
                ...
                {show?.id ? (
                    <div className="relative w-full h-[621px] md:h-auto overflow-hidden">
                        ...
                        <div className="bg-gradient-to-r from-black to-transparent flex justify-center items-center bg-[linear-gradient(to_top,black_15%,transparent_80%)] absolute top-0 bottom-0 right-0 left-0">
                            <Link href={"#episodes"} className="bg-[#5c00cc] p-6 rounded-full cursor-pointer">
                        ...
                        <div className="absolute z-[999] text-start bottom-4 md:bottom-10 lg:bottom-26 mx-4  flex  gap-4 items-end ">
                            ...
                            <div className="hidden lg:block ">
                                <Image
                                    src={`${imgSrc}${seasonInfo?.poster_path}`}
                                    onError={handleError}
                                    alt={`${show?.name}`}
                            ...
                            <div className="flex flex-col ">
                                ...
                                <div className="flex gap-3 mt-2">
                                    <Link href={`#episodes`} className=" rounded-xl px-2 md:px-5 py-2 md:py-3 flex gap-2 hover:opacity-80 duration-200 bg-[#5c00cc]">
                ...
                <div className="mx-4 mt-2">
                    ...
                    <div>
                        ...
                        {show?.id ? (
                            <p className="mt-2">{seasonInfo?.overview} </p>
                        ) : (
                ...
                <div className="mx-4 font-bold mt-5">
                    <h2 className="text-2xl " id="episodes">Episodes</h2>

                    <div className="flex gap-3 mt-2 overflow-x-scroll hide-scrollbar">
                        {
                            seasonInfo?.episodes?.length > 0 ? seasonInfo?.episodes?.map((p, i) =>
                                <Link href={`/stream/tv/${show?.id}/${seasonInfo?.season_number}/${p?.episode_number}`} key={i}
                                    className="relative shrink-0 cursor-pointer overflow-hidden w-72 ">
                                    <div className="overflow-hidden rounded-xl">
                                        <img src={`https://image.tmdb.org/t/p/w500${p?.still_path}`}
                                            className="w-72 scale-110 hover:scale-100 duration-300" alt="" />
                                    </div>
